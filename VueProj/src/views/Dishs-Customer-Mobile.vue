<!--点餐（顾客和用户均可）-->
<template>
<div style="background:rgb(241, 217, 207)">
  <div style="padding:10px;height:40px;width:100%;">
    <!--    顶部功能区-->
    <div style="margin:15px; font-size: 0.3rem">
      <div>{{ckid}}号{{cknum}}人{{cktype}}的顾客你好！
        预计消费:{{sumConsume}}元<el-button  @click="submit">提交</el-button>
        <!--这里要实现一下-->
    ...
    <!--    表格主体-->
    <div style=" margin-top:10px;margin-left:0px; width:100%; height:900px;" >
      <el-table
          :data="tableData"
          height="800"
        ...
        <!--      表格-->
         <el-table-column label="图片" header-align="center">
               ...
               　　　　 <img :src="require('../assets/img/'+scope.row.dimg)"  width="150" height="150"/>
               　　    <!-- <el-image
                                :src="require('../../assets/img/'+scope.row.dimg)"
                              ></el-image>-->
               </template>
               </el-table-column>
        <el-table-column v-if="false" sortable :sort-method="sortByIntDid" label="菜品号" prop="did" />
        
        <el-table-column style=""  align="left" label="基本信息">
          <template #default="scope">
            <div style="text-align: left;font-size: 0.4rem">
              <p>
                菜品名:{{scope.row.dname}}
              </p>
              <p>
                菜品类型：{{scope.row.dkind}}
              </p>
              <p>
                菜品规格： {{scope.row.dcount}}
              </p>
              <p>
                菜品单价：{{scope.row.dprice}}
              </p>
              <p>
                菜品数量：{{scope.row.dcount}}
              </p>
            </div>

<!--            <p>-->
<!--              菜品描述：{{scope.row.ddesc}}-->
<!--            </p>-->
          </template>
        </el-table-column>
...
<!--    <el-table-column v-if="false" sortable label="供应状态" prop="dstatus" />&ndash;&gt;-->
<!--        <el-table-column label="数量" prop="dcounts" />-->
<!--        <el-table-column label="餐品描述" prop="ddesc" />-->
          ...
          <template   #default="scope">
            <div>
              <el-button
                  ...
                  style="width: 90%; text-align: center;font-size: 0.5rem; background-color: #e04300;color: white"
                  @click="handleAdd(scope.$index, scope.row)">+</el-button>
            </div>
          <div>
            <el-button

                style="width: 90%; text-align: center;font-size: 0.5rem;background-color: #515151;color: white"
    ...
    <!--      新增与修改的弹框-->
    <el-dialog v-model="dialogVisible" title="确认订单" width="100%">
      <el-table
          :data="orderData"
...
export default {
  ...
  methods: {
    ...
    submit(){
      if(this.ckid!=null||this.ckid)//点单确认是否已选桌子
        this.dialogVisible = true;
      else
      {
        alert('请先定餐桌');
    ...
    save() {
      ...
      for(var i=0;i<this.orderData.length;i++)
      {
        if(this.orderData[i].dcounts!=0){
          ...
          console.log('Dish-Customer-1',this.oid);
          params.append('oid',this.oid);
          console.log('Dish-Customer-2',sessionStorage.getItem("did"))
