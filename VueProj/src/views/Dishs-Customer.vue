...
<div style="background:rgb(241, 217, 207)">
  <div style="padding:10px;height:40px;width:100%;">
    ...
    <div style="margin:15px;">
      ...
      <!--这里要实现一下-->
      {{ckid}}号{{cknum}}人{{cktype}}的顾客你好！
        预计消费：{{sumConsume}} 元
      </div>
    </div>
    <!--    表格主体-->
    <div style=" margin-top:10px;margin-left:0px; width:1980px; height:900px;" >
      <el-table
          :data="tableData"
          height="800"
        ...
        <!--      表格-->
         <el-table-column label="图片" header-align="center">
               ...
               　　　　 <img :src="require('../assets/img/'+scope.row.dimg)"  width="200" height="200"/>
               　　    <!-- <el-image
...
export default {
  ...
  methods: {
    ...
    reinit() {
      this.orderData=[];
      axios
          .get('http://192.168.1.102:8081/SAD')
          .then(response => {
    ...
    submit(){
      ...
      {
        alert('请先定餐桌');
        window.location.replace("192.168.1.102:8081:8080/room/desk-customer");
      }
    },
    save() {
      ...
      for(var i=0;i<this.orderData.length;i++)
      {
        if(this.orderData[i].dcounts!=0){
          ...
          axios.post("http://192.168.1.102:8081/insRD", params).then(response => {
            if (response.data.result == "success") {
              this.$message({
                type: "success",
                message: "下单成功！"
