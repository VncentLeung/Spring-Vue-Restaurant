...
<!--        预计消费：{{sumConsume}} 元-->
    ...
    <!--    表格主体-->
    <div style="overflow:scroll; width:100%; height:800px;font-size: 0.4rem;text-align: left" >
      <el-table
          :data="table_data"
            :header-cell-style="{ color:'#b45c41', background:'rgb(241, 217, 207)' }"
        :cell-style="{  background:'rgb(241, 217, 207)' ,color:'black'}"
          border v-if="!needChooseDesk" style="width: 100%">

        <el-table-column label="图片" header-align="center">
          　<template #default="scope">
          　　　　 <img :src="require('../assets/img/img-'+scope.row.ktype+'.png')"  width="150" height="150"/>
          　　    <!-- <el-image
      ...
      <el-table
        ...
        <el-table-column v-if="false" sortable :sort-method="sortByIntDid" label="桌号" prop="kid" />
        <el-table-column label="图片" header-align="center">
          　<template #default="scope">
          　　　　 <img :src="require('../assets/img/img-'+scope.row.ktype+'.png')"  width="150" height="150"/>
          　　    <!-- <el-image
...
export default {
  ...
  data() {
    return {
      ...
      ,
      cid:
          (sessionStorage.getItem("cid"))==null? this.$router.push("/loginc"):(sessionStorage.getItem("cid")).toString()
      ,
      ctel:
  ...
  methods: {
    ...
    submit(){
      ...
      {
        alert('请先定餐桌');
        window.location.replace("192.168.1.104:8081:8080/customer/desk-customer-mobile");
      }
    },
    save() {
      // 验证后提交
      ...
      //   if(this.orderData[i].dcounts!=0){
       ...
       axios.get("http://192.168.1.104:8081/SARcsu").then(response => {
        if (response.data.result == "success") {
          ...
          this.$router.push("/customer/dishs-customer-mobile");
          //this.reinit();
        } else {
          alert(response.data.reason);
        }
